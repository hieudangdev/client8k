...
use Kho8k\Core\Requests\MovieRequest;
 ...
 * @package Kho8k\Core\Controllers\Admin
...
class MovieCrudController extends CrudController
{
    ...
    {
        CRUD::setModel(\Kho8k\Core\Models\Movie::class);
        CRUD::setRoute(config('backpack.base.route_prefix') . '/movie');
        CRUD::setEntityNameStrings('movie', 'movies');
        CRUD::setCreateView('kho8k::movies.create',);
        CRUD::setUpdateView('kho8k::movies.edit',);
    }
    ...
    protected function setupListOperation()
    {
        ...
        $this->crud->addFilter([
            ...
            if ($field === 'language') {
                ...
                $this->crud->query->where($field, '')->orWhere($field, NULL)->orWhere($field, 'like', '%img.kho8k%')->orWhere($field, 'like', '%img.hiephanhthienha%');
            }
        });
        ...
        CRUD::addColumn([
            ...
            'label' => 'Thông tin',
            'type' => 'view',
            'view' => 'kho8k::movies.columns.column_movie_info',
    ...
    private function processJsonData()
    {
        ...
        if ($entry && $entry->tags) {
            ...
            return implode("\n", $names); // Trả về danh sách tên, mỗi tên trên một dòng
        }
